...
def cat_to_one_hot(y_cat,cat2ind):
    '''
...
def prepare_data(data_mat, data_path_src, save_dir, cat2ind,ind2cat, dataset_type='train', generate_npy=False, debug_mode=False,image_size=(224,224), body_size =(128,128)):
  '''
    ...
    for person in range(nop):
      ...
      try:
        ...
        if not os.path.exists(image_path):
          ...
          context_cv = cv2.resize(context, image_size)
      ...
      if generate_npy == True:
        ...
        if dataset_type == 'train':
          cat_arr.append(cat_to_one_hot(et.cat,cat2ind))
          cont_arr.append(np.array(et.cont))
        else: 
          cat_arr.append(cat_to_one_hot(et.comb_cat,cat2ind))
